...
def _post_request(url: str, request_params, logger):
    ...
    try:
        ...
        logger.exception(
            f'HTTPError at {full_url} - {err.response.status_code} {err.response.reason} {err.response.json()} - aborting read\nError: {err}')
        return None
    except Exception as e:
        if hasattr(e, 'response'):
            logger.exception(
                f'Error at {full_url} {e.response.status_code} {e.response.reason} {e.response.json()} - aborting read\nError: {e}')
        else:
...
class Authenticate:
    ...
    def __init__(self, config: AppConfig, plugin_name, client_id: str, client_secret: str, username: str,
                 password: str) -> None:
...
class Plugin(InputPluginBase):
    ...
    def _read_configuration(self, config: AppConfig):
        ...
        if self._module_name is None:
            self._logger.debug(
                "No outdoor module name supplied => use the first module type: '%s' found attached to the station",
                _OUTDOOR_MODULE_TYPE)
        else:
    ...
    def _find_station(self, stations):
        ...
        if station is None:
            ...
            raise ModuleNotFound(
                f"Station not found by {find_by} - station list: [{', '.join(station_names_and_types)}]")
    ...
    def _read_metrics(self):
        ...
        if self._simulation:
            ...
            return ([Metric(self._zone, temp)], text_temperatures)

        try:
            access_token = Authenticate(self._config, self.plugin_name, self._client_id, self._client_secret,
                                        self._username, self._password).access_token()
